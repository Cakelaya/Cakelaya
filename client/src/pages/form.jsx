...
const Form = () => {
  ...
  useEffect(() => {
    ...
    // rayzorpay Button Used
    if (!rzpPaymentForm.hasChildNodes()) {
      const script = document.createElement("script");
      script.src = "https://checkout.razorpay.com/v1/payment-button.js";
      script.async = true;
      script.dataset.payment_button_id = (category == "Chaats and Juices") ? "pl_LmKwNZl6Y0NWwc" : "pl_L9wD1Z9W3Ucdvl";
  ...
  const onHandle = async () => {
    ...
    let add = address + ", " + landmark + ", " + city + ", " + state + ", " + country + ", " + pincode;

    const currentTime = new Date();
    const LocalTime = currentTime.toLocaleString('en-US', { hour: 'numeric', minute: 'numeric', hour12: true });
    const currentDate = Date().toLocaleString();
    const orderTime = currentDate.split(" ");
    const captureTime = LocalTime + ', ' + orderTime[2] + " " + orderTime[1] + " " + orderTime[3];
        ...
        <Summary>
          ...
          <SummaryItem type="total" style={{ marginBottom: "49px" }}>
            <SummaryItemText>Total</SummaryItemText>
              {city=="Noida" && <SummaryItemPrice>₹ {cart.total + 50 } </SummaryItemPrice>} 
              {city=="Sultanpur" && <SummaryItemPrice>₹ {cart.total + 0 } </SummaryItemPrice>} 
          </SummaryItem>
